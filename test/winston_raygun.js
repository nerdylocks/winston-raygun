...
describe('Winston Raygun transport', function(){
	...
	it('should add Raygun transport to winston.Transport', function(){
		chai.assert(winston.transports.hasOwnProperty('Raygun'));
	});

	it('should log multiple error object', function(done) {
		var error_object = { error_type: 'invalid_request', message: 'error message?'};
		winston.error('TEST ERROR!', error_object, 'Error', new Error('another'), 'string', false, 9, function (err) {
				if (err) {
					done(err);
				}
				done();
